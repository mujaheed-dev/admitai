...
import { LogOut, Shield, FileText, Trash2, AlertTriangle, CheckCircle, X } from 'lucide-react'
...
export default function ProfileMenu({ user, firstName, onSignOut, onGoToPrivacy, onGoToTerms, onDeleted }) {
  const [isOpen,       setIsOpen]       = useState(false)
  const [step,         setStep]         = useState('menu')  // 'menu' | 'delete-confirm' | 'delete-success'
  ...
  useEffect(() => {
    function handler(e) {
      if (e.key === 'Escape' && isOpen && !deleting) close()
    }
  ...
  function close() {
    if (deleting) return
    setIsOpen(false)
    // Reset delete flow after the dropdown has closed (avoids flash)
    setTimeout(() => { setStep('menu'); setConfirmText(''); setDeleteError('') }, 200)
  }
